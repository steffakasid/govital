= Govital Configuration Guide

== Overview

Govital supports flexible configuration through multiple methods with a clear priority system. Configuration determines how dependencies are classified as "healthy" (active) or "stale" (inactive).

== Configuration Options

=== Scanner Configuration

==== `stale_threshold_days`

* *Description*: Number of days a dependency can be inactive before being marked as stale
* *Type*: Integer
* *Default*: 30 days
* *Recommendation*:
  - 30 days: Very aggressive (flags most inactive dependencies)
  - 90 days: Strict (good for fast-moving projects)
  - 180 days: Moderate (balanced for most projects)
  - 365 days: Lenient (accepts stable, mature libraries)
  - 730 days: Very lenient (only flags abandoned projects)

==== `active_threshold_days`

* *Description*: Number of days a dependency must have been updated within to be considered actively maintained
* *Type*: Integer
* *Default*: 90 (3 months)
* *Note*: Currently a placeholder for future enhancement (not actively used yet)

==== `include_indirect_dependencies`

* *Description*: Whether to include indirect (transitive) dependencies in the scan
* *Type*: Boolean
* *Default*: `false` (only scan direct dependencies)
* *Note*: Including indirect dependencies can significantly increase scan time for large projects

==== `log_level`

* *Description*: Logging verbosity
* *Type*: String
* *Default*: `info`
* *Options*: `debug`, `info`, `warn`, `error`

==== `acknowledged_dependencies`

* *Description*: List of module paths to acknowledge as inactive without marking as errors
* *Type*: Array of strings
* *Default*: empty list
* *Example*: 
  - `github.com/user/legacy-lib`
  - `github.com/company/internal-tool`
* *Note*: Acknowledged dependencies are marked with ⊘ symbol and not counted in the inactive count

== Configuration Methods

=== 1. CLI Flags (Highest Priority)

Override any setting directly on the command line:

[source,bash]
----
# Override stale threshold
govital scan --stale-threshold 180

# Override log level
govital scan --log-level debug

# Combine with other flags
govital scan --project-path /path/to/project --stale-threshold 90 --log-level debug
----

Available flags:

* `-t, --stale-threshold int`: Days before marking as stale (default 30)
* `-i, --include-indirect`: Include indirect (transitive) dependencies (default false)
* `-w, --workers int`: Number of parallel workers for scanning (default 4)
* `-p, --project-path string`: Path to scan (default ".")
* `-l, --log-level string`: Logging level (default "info")

=== 2. Configuration File

Create `govital.yaml` in one of these locations (checked in order):

. Current directory (`.`)
. `$HOME/.govital/`
. `$HOME/.config/govital/`
. `/etc/govital/`

Example configuration:

[source,yaml]
----
# Log level
log_level: info

# Scanner settings
scanner:
  # Days before marking dependency as stale
  stale_threshold_days: 30
  
  # Days for active maintenance threshold
  active_threshold_days: 90

  # Include indirect (transitive) dependencies
  include_indirect_dependencies: false

  # List of dependencies to acknowledge as inactive
  acknowledged_dependencies:
    - golang.org/x/net
    - github.com/legacy/package
----

=== 3. Environment Variables

Future support planned. Currently not implemented but reserved for:

* `GOVITAL_STALE_THRESHOLD_DAYS`
* `GOVITAL_ACTIVE_THRESHOLD_DAYS`
* `GOVITAL_LOG_LEVEL`

== Configuration Priority (Highest to Lowest)

. **CLI Flags** - Use these to override for one-off scans
. **Config File** - Use for project-level or user-level defaults
. **Environment Variables** - Future support
. **Built-in Defaults** - Fallback values

== Examples

=== Strict Project Policy (No Stale Dependencies)

[source,yaml]
----
# .govital.yaml in project root
log_level: warn

scanner:
  stale_threshold_days: 30
----

Usage:

[source,bash]
----
govital scan
----

=== Lenient Corporate Standard (Allows Mature Libraries)

[source,yaml]
----
# /etc/govital/govital.yaml
log_level: info

scanner:
  stale_threshold_days: 730  # 2 years
----

Usage:

[source,bash]
----
govital scan /path/to/project
----

=== One-Time Audit with Custom Threshold

[source,bash]
----
govital scan --stale-threshold 180 --log-level debug
----

=== Override Config File with CLI

If `/etc/govital/govital.yaml` specifies 365 days but you want 180:

[source,bash]
----
govital scan --stale-threshold 180
# The CLI flag overrides the config file setting
----

== Understanding Results

When you run `govital scan`, it reports:

[source]
----
Stale Threshold: 30 days

Summary:
  Total Dependencies:        32
  Inactive Dependencies:     28
  Errors:                    0

Dependencies:
  - github.com/user/pkg@v1.2.3 [✗ Inactive] (last commit: 404 days ago)
  - github.com/other/lib@v2.1.0 [✓ Active] (last commit: 15 days ago)
----

* *Stale Threshold*: Shows the current setting (from CLI, config file, or default)
* *✓ Active*: Last commit within threshold (e.g., < 30 days ago)
* *✗ Inactive*: Last commit exceeded threshold (e.g., > 30 days ago)
* *Days ago*: Calculated from module release date to today

== Common Use Cases

=== Case 1: New Project - Strict Standards

Want to ensure all dependencies are actively maintained:

[source,bash]
----
# One-time check with 90 days threshold
govital scan --stale-threshold 90
----

=== Case 2: Acknowledge Specific Inactive Dependencies

You have some dependencies that are intentionally inactive but acceptable (e.g., stable/mature libraries or internal legacy packages):

[source,yaml]
----
# ~/.govital/govital.yaml
scanner:
  stale_threshold_days: 180
  
  # These dependencies won't count as "Inactive" in the results
  acknowledged_dependencies:
    - golang.org/x/sync
    - github.com/company/internal-api
    - github.com/deprecated/legacy-lib
----

Result output:

[source]
----
Summary:
  Total Dependencies:        10
  Inactive Dependencies:     2 (Direct: 1, Indirect: 1)
  Acknowledged:              3 (Direct: 2, Indirect: 1)
  ...

Dependencies:
  - golang.org/x/sync@v0.1.0 [⊘ Acknowledged] (last release: 200 days ago)
  - github.com/company/internal-api@v1.0.0 [⊘ Acknowledged] (last release: 150 days ago)
----

=== Case 3: Legacy Project - Mature Dependencies

Project uses stable, mature libraries that update infrequently:

[source,bash]
----
# Create ~/.govital/govital.yaml
cat > ~/.govital/govital.yaml << 'EOF'
scanner:
  stale_threshold_days: 730
EOF

govital scan
----

=== Case 4: CI/CD Pipeline Audit

Run in CI with specific policy:

[source,bash]
----
# In your CI pipeline
govital scan --stale-threshold 365 --log-level warn

# Check for inactive dependencies and fail if found
INACTIVE=$(govital scan --stale-threshold 365 2>/dev/null | grep "✗ Inactive" | wc -l)
if [ $INACTIVE -gt 5 ]; then
  exit 1
fi
----

== Troubleshooting

=== Config file not being read

. Verify location (current dir, `~/.govital/`, or `/etc/govital/`)
. Check file is named `govital.yaml` (not `.yml` or other formats)
. Run with `--log-level debug` to see config loading attempts:
+
[source,bash]
----
govital scan --log-level debug
----

=== Unexpected threshold behavior

. Check CLI flags override config file - verify no flags are set
. View current settings with:
+
[source,bash]
----
govital scan --log-level debug
----

=== All dependencies marked as inactive

. Your threshold may be too strict
. Try increasing threshold:
+
[source,bash]
----
govital scan --stale-threshold 365
----

== Performance Tuning

=== Parallel Scanning

Govital uses a worker pool pattern to scan dependencies in parallel. The number of workers can be configured via the `--workers` flag:

* **Default (4 workers)**: Good for most systems and workloads
* **High-end systems**: Use 8-16 workers for better performance
* **Resource-constrained**: Use 1-2 workers to minimize CPU/memory usage

Example:

[source,bash]
----
# Fast scan on powerful hardware
govital scan --workers 16

# Conservative scan on limited hardware
govital scan --workers 1
----

Performance impact depends on:
* Number of dependencies (more deps = better parallelism benefit)
* System CPU cores (more cores = higher optimal worker count)
* Network latency (downloading module info involves network calls)
[source,bash]
----
govital scan --stale-threshold 365
----
